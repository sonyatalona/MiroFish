...
// Get maxRounds from query params at init time so child components receive it immediately
...
const handleGoBack = async () => {
  // Before going back to Step 2, shut down any running simulation
  ...
  // Navigate back to Step 2 (Environment Setup)
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh should not show full-screen loading to avoid flicker
...
onMounted(() => {
  ...
  // Log maxRounds config (value already retrieved from query params at init)
  if (maxRounds.value) {
