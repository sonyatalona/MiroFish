            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but entire section may not be complete yet) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract relation chains - extract all, no limit
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **alumni_345 (index=1)**: As an alumnus...
        ...
        if (name) {
          ...
          // Continuation of reason (excluding closing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatibility: only one platform marker case)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display only if not placeholder
          ...
          // No per-platform markers (very old format), treat entire text as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compatible with multiple quotation mark formats)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer matching > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" or > \u201Ctext\u201D (CJK quotation marks)
        if (!quoteMatches) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder text) and content is different
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx), since section title is already displayed in the outer layer
