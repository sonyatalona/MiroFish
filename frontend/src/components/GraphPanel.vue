...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull towards center to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from node's right side, loop around and back
    ...
    // Offset perpendicular to line direction, calculated by distance ratio for visible curves
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if dragging has truly started (moved past threshold)
