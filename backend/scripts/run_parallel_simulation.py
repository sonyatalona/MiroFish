...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr to UTF-8 (fix console encoding issues)
...
class ParallelIPCHandler:
    ...
    Manages environments for both platforms, processes Interview commands
    """
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Interviews requiring both platforms
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "Environment is about to shut down"})
            return False
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of codenames like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and enrich with full context information
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Enrich context information (post content, usernames, etc.)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: enrich with original post content, author, and quote comment
            ...
            # Get the quote content of the quoted post
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: enrich with commented post information
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    improving concurrency capability.
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional, used to truncate excessively long simulations)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, used to truncate excessively long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: after simulation completes, do not exit but wait for interview commands.
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify the asyncio loop to exit (giving it a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
