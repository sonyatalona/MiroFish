...
class SimulationRunner:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end events, mark platform as completed
    ...
    def get_all_actions(
        ...
        # If per-platform files do not exist, try reading legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format files should have platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def register_cleanup(cls):
        ...
        # If not in debug mode, this env var does not exist, cleanup still needs to be registered
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if environment is alive, False if shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get detailed status of the simulation environment
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview on both platforms in dual-platform simulation, return integrated result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: Interview on both platforms for each Agent in dual-platform simulation
            timeout: Timeout in seconds
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specific Agent ID (optional, only get that Agent's history)
