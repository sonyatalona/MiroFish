...
def create_simulation():
    ...
    Note: Parameters such as max_rounds are intelligently generated by the LLM; no manual setup required
...
def get_simulation_history():
    ...
    Used for homepage historical project display. Returns a simulation list with rich info
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get file list from associated project (up to 3)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generation is in progress)
    - Can return partial information even if configuration is not fully generated
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (complete configuration generated by LLM)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "Simulation configuration not found. Please call the /prepare endpoint first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file not found. Please call the /prepare endpoint first"
            }), 404
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (shared scripts located in backend/scripts/)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if a simulation is running or completed, it will first stop and clean up run logs
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows content from the latest round across both platforms
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (entered command-waiting mode after completing simulation cycles)
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation, allowing it to gracefully exit command-waiting mode.

    Note: This differs from the /stop endpoint. /stop forcefully terminates the process,
    while this endpoint lets the simulation gracefully shut down the environment and exit.
