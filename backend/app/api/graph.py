...
def reset_project(project_id: str):
    """
    Reset project state (for rebuilding the knowledge graph)
    """
    project = ProjectManager.get_project(project_id)
...
def build_graph():
    """
    Endpoint 2: Build knowledge graph based on project_id
    ...
    try:
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep to finish processing (query processed status for each episode)
                task_manager.update_task(
                    task_id,
